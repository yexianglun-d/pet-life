<template>
  <section class="page-section community-page">
    <div class="pet-admin-hero">
      ...
      <h1 class="page-section__title">问答内容治理</h1>
    ...
    <article class="pet-admin-panel community-section">
      <div class="community-toolbar">
        ...
        <div class="community-toolbar__actions">
          ...
          <el-input v-model="filters.keyword" size="small" class="community-keyword" placeholder="问题关键词" clearable />
      ...
      >
        <el-table-column label="问题内容" min-width="360">
          <template #default="{ row }">
            <div class="community-cell">
              ...
              <div class="community-cell__detail">{{ summarizeText(row.content, 112) }}</div>
        ...
        <el-table-column label="回答 / 互动" width="150">
          <template #default="{ row }">
            <div class="community-count-stack">
              <span><strong>{{ row.comment_count }}</strong> 回答</span>
          ...
          <template #default="{ row }">
            <div class="community-actions">
              ...
              <el-button
                ...
                :loading="processingQuestionId === row.post_id"
      ...
      <div class="community-toolbar">
        ...
        <div class="community-toolbar__actions">
          <el-select v-model="auditLogFilters.action" size="small" class="community-audit-filter" placeholder="动作">
            ...
            <el-option label="问答下架" value="community_question_take_down" />
            <el-option label="问答恢复" value="community_question_restore" />
          </el-select>
    ...
    <el-drawer v-model="detailDrawerVisible" title="问答详情" size="580px">
      <div v-loading="detailLoading" class="community-detail">
        <template v-if="activeQuestionDetail">
          <section class="community-detail__section">
            <div class="community-detail__heading">
              <h3>{{ activeQuestionDetail.question.title }}</h3>
              <el-tag :type="reviewStatusTagType(activeQuestionDetail.question.review_status)">
                {{ reviewStatusLabel(activeQuestionDetail.question.review_status) }}
              </el-tag>
            </div>
            <p class="community-detail__content">{{ activeQuestionDetail.question.content }}</p>
            <div class="community-detail__tags">
              <el-tag type="info">问答</el-tag>
              <el-tag>{{ visibilityLabel(activeQuestionDetail.question.visibility) }}</el-tag>
            </div>
          </section>
          ...
          <section class="community-detail__section">
            ...
            <dl class="community-detail-list">
              <div>
                <dt>作者</dt>
                <dd>{{ activeQuestionDetail.question.author.nickname || '-' }} · {{ activeQuestionDetail.question.author.user_id }}</dd>
              </div>
              <div>
                <dt>宠物</dt>
                <dd>
                  {{
                    activeQuestionDetail.question.pet
                      ? `${activeQuestionDetail.question.pet.pet_name} · ${petTypeLabel(activeQuestionDetail.question.pet.pet_type)}`
                      : '-'
                  }}
                  <span v-if="activeQuestionDetail.question.pet?.breed">
                    · {{ activeQuestionDetail.question.pet.breed }}
                  </span>
                </dd>
              </div>
              <div>
                <dt>话题</dt>
                <dd>{{ activeQuestionDetail.question.topic?.topic_name || '-' }} · {{ activeQuestionDetail.question.topic?.topic_id || '-' }}</dd>
              </div>
              <div>
                <dt>城市</dt>
                <dd>{{ activeQuestionDetail.question.city_code || '-' }}</dd>
              </div>
              <div>
                <dt>发布时间</dt>
                <dd>{{ formatDateTime(activeQuestionDetail.question.published_at) }}</dd>
              </div>
            </dl>
          </section>
          ...
          <section class="community-detail__section">
            <h3>回答列表</h3>
            <div v-if="activeQuestionDetail.answers.length === 0" class="community-empty-inline">
              暂无回答
            </div>
            <div v-else class="community-answer-list">
              <article
                v-for="answer in activeQuestionDetail.answers"
                :key="answer.comment_id"
                class="community-answer"
              >
                <div class="community-answer__title">
                  <span>{{ answer.author.nickname || answer.author.user_id }}</span>
                  <el-tag size="small" type="info">#{{ answer.comment_id }}</el-tag>
                </div>
                <p>{{ answer.content }}</p>
                <div class="community-cell__meta">{{ formatDateTime(answer.created_at) }}</div>
              </article>
            </div>
          </section>
          ...
          <section class="community-detail__section">
            <el-button
              :type="activeQuestionDetail.question.review_status === 'rejected' ? 'primary' : 'danger'"
              :loading="processingQuestionId === activeQuestionDetail.question.post_id"
              @click="openStatusDialog(activeQuestionDetail.question)"
            >
              {{ activeQuestionDetail.question.review_status === 'rejected' ? '恢复问答' : '下架问答' }}
            </el-button>
          </section>
        </template>
      </div>
    ...
    <el-dialog v-model="statusDialogVisible" :title="statusDialogTitle" width="560px">
      <div v-if="activeStatusQuestion" class="community-dialog-context">
        <div class="community-dialog-context__title">
          {{ activeStatusQuestion.title }} · #{{ activeStatusQuestion.post_id }}
        </div>
        <div class="community-dialog-context__meta">
          当前状态：{{ reviewStatusLabel(activeStatusQuestion.review_status) }} · 作者：{{ activeStatusQuestion.author.nickname || activeStatusQuestion.author.user_id }}
        </div>
      </div>
...
import {
  getAdminCommunityQuestion,
  listAdminCommunityQuestions,
  listCommunityAuditLogs,
  updateAdminCommunityQuestionStatus,
...
const questions = ref<CommunityPostSnapshot[]>([]);
...
const processingQuestionId = ref<string | null>(null);
const activeQuestionDetail = ref<CommunityQuestionDetailSnapshot | null>(null);
const activeStatusQuestion = ref<CommunityPostSnapshot | null>(null);
...
const approvedCount = computed(
  () => questions.value.filter((question) => question.review_status === 'approved').length
);
const pendingCount = computed(
  () => questions.value.filter((question) => question.review_status === 'pending_review').length
);
const rejectedCount = computed(
  () => questions.value.filter((question) => question.review_status === 'rejected').length
);
const answerCount = computed(() => questions.value.reduce((total, question) => total + question.comment_count, 0));
const totalInteractions = computed(() =>
  questions.value.reduce(
    (total, question) => total + question.like_count + question.comment_count + question.favorite_count,
    0
  )
);
const statusDialogTitle = computed(() => `${governanceActionLabel(statusForm.action)}问答内容`);
const summaryCards = computed(() => [
  {
    title: '当前问答',
    description: '当前筛选条件下返回的问答数量。',
    value: `${questions.value.length} 条`
  },
  {
    title: '回答线索',
    description: '列表中问答累计回答数量。',
    value: `${answerCount.value} 条`
  },
  {
    title: '已下架',
    description: '当前处于下架状态的问答。',
    value: `${rejectedCount.value} 条`
  },
  {
    title: '互动总量',
    description: '点赞、回答和收藏的合计值。',
...
async function loadPage() {
  await Promise.all([loadQuestions(), loadAuditLogs()]);
}
...
async function loadQuestions() {
  ...
  errorMessage.value = '';
  try {
    questions.value = await listAdminCommunityQuestions({
    ...
    errorMessage.value = error instanceof Error ? error.message : '问答列表加载失败';
  } finally {
...
async function loadAuditLogs() {
  ...
  try {
    ...
    auditLogErrorMessage.value = error instanceof Error ? error.message : '问答治理审计加载失败';
  } finally {
    auditLogLoading.value = false;
  }
}

async function openDetail(question: CommunityPostSnapshot) {
  detailDrawerVisible.value = true;
  activeQuestionDetail.value = {
    question,
    answers: []
  };
  detailLoading.value = true;
  try {
    activeQuestionDetail.value = await getAdminCommunityQuestion(question.post_id);
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : '问答详情加载失败');
  } finally {
    detailLoading.value = false;
  }
}

function openStatusDialog(question: CommunityPostSnapshot) {
  activeStatusQuestion.value = question;
  statusForm.action = question.review_status === 'rejected' ? 'restore' : 'take_down';
  statusForm.adminNotes = '';
  statusDialogVisible.value = true;
}

async function submitStatusForm() {
  if (!activeStatusQuestion.value) {
    return;
  }

  processingQuestionId.value = activeStatusQuestion.value.post_id;
  statusSubmitting.value = true;
  try {
    const updatedQuestion = await updateAdminCommunityQuestionStatus(
      activeStatusQuestion.value.post_id,
      statusForm.action,
      normalizeNullableText(statusForm.adminNotes)
    );
    questions.value = questions.value.map((question) =>
      question.post_id === updatedQuestion.post_id ? updatedQuestion : question
    );
    if (activeQuestionDetail.value?.question.post_id === updatedQuestion.post_id) {
      activeQuestionDetail.value = {
        ...activeQuestionDetail.value,
    ...
    ElMessage.error(error instanceof Error ? error.message : '问答状态更新失败');
  } finally {
    statusSubmitting.value = false;
    processingQuestionId.value = null;
  }
}
...
function auditActionLabel(action: string) {
  const labelMap: Record<string, string> = {
    community_question_take_down: '问答下架',
    community_question_restore: '问答恢复'
  };
