...
1. 当前交付目标覆盖 `iOS + Android App` 用户端、服务端和后台管理端。
...
1. 当前交付包含用户端 App、用户端所需服务端接口，以及审核/举报/服务商维护等后台管理能力。
2. 商城仅实现用户端占位页，不进入 `commerce` 模块的后端实现批次。
3. `device` 模块停留在接口边界和数据结构预留，不接入厂商 SDK、云回调或事件链路。
4. 审核、举报处理、服务商数据维护采用后台 Web 承接，不追求一次性做成完整运营平台。
...
当前阶段同步建设后台 Web，覆盖审核、举报处理、服务商维护和基础配置能力。
...
5. 服务城市未开通
兜底：按 `service_city_configs` 的未开通原因返回明确说明，不返回空白。
