...
CREATE TABLE IF NOT EXISTS `moderation_tasks` (
  ...
  `media_asset_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '关联媒体 ID',
  `review_vendor` VARCHAR(30) DEFAULT NULL COMMENT '审核服务商',
  `review_type` VARCHAR(20) NOT NULL COMMENT '审核类型：text/image/video',
  `request_payload` JSON DEFAULT NULL COMMENT '审核请求快照',
  `review_result` JSON DEFAULT NULL COMMENT '审核结果',
  `review_status` VARCHAR(20) NOT NULL DEFAULT 'pending' COMMENT '状态：pending/passed/rejected/failed',
  ...
  KEY `idx_moderation_tasks_media` (`media_asset_id`),
  KEY `idx_moderation_tasks_status` (`review_status`, `created_at`),
  CONSTRAINT `fk_moderation_tasks_media` FOREIGN KEY (`media_asset_id`) REFERENCES `media_assets` (`id`)
...
-- DDL 结束
-- 说明：
-- 1. 该版本为评审级草案，适合作为 Flyway/Liquibase 初始版本参考。
