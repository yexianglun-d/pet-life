...
- 当前阶段服务端使用 `BootstrapMemoryStore` 作为开发期内存适配器，统一承接用户、家庭、宠物、健康记录、提醒、萌宠日常等数据。
- 已修复测试间状态污染问题，内存基线数据支持显式重置。
...
1. 服务端接入真实鉴权与数据库持久化，替换开发期内存适配器。
2. 移动端继续补登录发送验证码、退出登录、接口异常态处理。
3. 后台接入审核中心、服务商管理、系统配置的真实接口。
4. 建立首批数据库迁移脚本和环境配置。
